 ...
 */
export function parsePageLinks(html, origin = PORTAL_ORIGIN) {
    ...
    for (const a of root.querySelectorAll("a")) {
        ...
        if (!lm)
            continue;
        if (/[?&]sortbyn=/.test(inner))
            continue; // skip sort-order toggles
        const lstart = parseInt(lm[1], 10);
        if (!byStart.has(lstart))
            byStart.set(lstart, absoluteUrl(href, origin));
    }
...
export function germanToIso(dmy) {
 ...
 */
export function parseList(html, opts = {}) {
    ...
    for (const row of root.querySelectorAll("ul#tabellen_ul li.tabzeile")) {
        ...
        const struckThrough = /line-through/.test((cells[1].getAttribute("style") ?? "") + (anchor.getAttribute("style") ?? ""));
        events.push({
            id,
            slug,
            type,
...
function parseStartLocation(td) {
 ...
 */
export function parseDetail(html, id, detailUrl) {
    const root = parse(html);
    ...
    const cancelMatch = html.match(/Die Veranstaltung wurde abgesagt<\/strong>(?:<br\s*\/?>)?([^<]*)/i);
